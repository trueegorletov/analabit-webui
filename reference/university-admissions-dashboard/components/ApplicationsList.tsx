  ...
  const renderAdmission = (app: Application) => {
    switch (app.admission) {
      ...
      case AdmissionDecision.NOT_COMPETING_TEXT:
        return (
          <span className="text-yellow-500 font-medium">Not competing</span>
        );
        ...
        <table className="min-w-full text-sm">
          <thead>
            <tr className="border-b border-gray-700">
              <th className="py-2 px-3 text-left font-medium text-gray-400 sticky top-0 bg-[#1C1C22] z-10">
                Rank
              </th>
              <th className="py-2 px-3 text-left font-medium text-gray-400 sticky top-0 bg-[#1C1C22] z-10">
                Student ID
              </th>
              <th className="py-2 px-3 text-right font-medium text-gray-400 sticky top-0 bg-[#1C1C22] z-10">
                Priority
              </th>
              <th className="py-2 px-3 text-right font-medium text-gray-400 sticky top-0 bg-[#1C1C22] z-10">
                Score
              </th>
              <th className="py-2 px-3 text-center font-medium text-gray-400 sticky top-0 bg-[#1C1C22] z-10">
                Orig, celt
              </th>
              <th className="py-2 px-3 text-center font-medium text-gray-400 sticky top-0 bg-[#1C1C22] z-10">
                Other unlv
          ...
          <tbody>
            {applications.map((app, index) => (
              ...
              >
                <td
                  className={`py-3 px-3 ${app.isCurrentUser ? 'text-green-400 font-bold' : 'text-gray-200'}`}
                >
                  {app.rank}
                </td>
